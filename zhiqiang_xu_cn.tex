...
\cventry{2022.04 -- 至今}{系统软件主任工程师}{极氪汽车（宁波杭州湾新区）有限公司}{上海市}{}{
  ...
  \item 极氪智能座舱SOA通信平台中间件架构方案设计。
    \begin{itemize}
    \item 基于开源vsomeip和commonapi，为极氪2.x平台车型开发SOA通信组件。
    \item 设计并实现基于Docker的开发和编译环境，保证每个目标版本编译环境的一致性。
      提供SOMEIP服务的单机通信仿真，以及在线单步调试能力。提升SOA 组件的开发效率。
    \item 带领团队设计并实现将通信矩阵设计文件转换为C++和Java代码的工具。从而
      在开发Native程序和Android程序时，能够基于该工具转换的模板工程，快速展开
      服务业务的开发。
    \item 带领团队设计并实现SOA 组件的SDK 化，SOA SDK可分别编译适配为支持 Linux
      ，QNX，Android等操作系统。从而实现通信组件的平台化能力。
    \item 开发SOMEIP 以太网报文解析工具。该工具根据通信矩阵定义文件，将SOMEIP 的
      以太网抓包解析为服务接口及对应的数据内容。在中间件开发和服务联调工作中，一
      定程度上提升了对服务数据确认的工作效率。
    \item 根据开发样机接口使用不便的痛点，设计多种业务联调方案和自动化工具，结
      合ADB，ROS路由器，将开发电脑接入样机网络，从而可以很方便地对服务进行仿真联
      调。
    \end{itemize}
  \item 极氪智能座舱ZKOS通信方案业务开发
    \begin{itemize}
    \item 验证ZKOS 通信方案的工具能力和通信框架能力。
    \item 根据3.0车型平台的SOA服务定义编写服务接口测试程序。
    \item 开发服务验证工具，简化使用ZKOS 开发，编译，部署以及运行测试程序的流程。
    \item 参与并指导对ZKOS SDK 的剥离方案。
    \end{itemize}
  \item 极氪智能座舱3.0平台诊断功能开发
    \begin{itemize}
    \item 3.0平台以太网诊断架构设计
    \item 3.0平台以太网诊断架构原型实现
    \item 3.0平台以太网诊断服务开发
    \end{itemize}
  \end{itemize}
}
